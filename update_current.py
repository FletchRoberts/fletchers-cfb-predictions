...
def main():
    ...
    # ---- Talent Composite, re-added as a 5th cold-start source ----
    # Originally removed because it can't reflect transfer portal turnover --
    # but on its own, removing it let real-performance sources (SP+/FPI/SRS/
    # Poll) overcorrect blue-blood programs during a single down year (LSU:
    # rank 43 despite being a top-25 program everywhere else). Re-added
    # alongside the others, not as a replacement -- Talent anchors roster
    # quality, the others capture recent performance, blended together.
    print("Fetching Talent Composite...")
    talent_raw = {r["team"]: r["talent"] for r in fetch("/talent", {"year": CURRENT_SEASON})
                  if r.get("talent") is not None}
    if len(talent_raw) < 20:
        print(f"  Only {len(talent_raw)} teams found in {CURRENT_SEASON} talent data -- "
              f"falling back to {PRIOR_SEASON}.")
        talent_raw = {r["team"]: r["talent"] for r in fetch("/talent", {"year": PRIOR_SEASON})
                      if r.get("talent") is not None}
    print(f"  Talent Composite: {len(talent_raw)} teams found "
          f"({'2026' if len(talent_raw) >= 20 else 'fallback'} data).")
